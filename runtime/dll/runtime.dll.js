...
var initialState={language:"en-US",entrypoint:null,ready:false};/* harmony default export */ const runtime = (function(){var state=arguments.length>0&&arguments[0]!==void 0?arguments[0]:initialState;var action=arguments.length>1?arguments[1]:void 0;switch(action.type){case platformfrom_dll_reference_platform_dll.constants.SET_LANGUAGE:{return{language:action.payload.language,entrypoint:state.entrypoint,ready:state.ready};}case platformfrom_dll_reference_platform_dll.constants.MODULES_READY:{return{language:state.language,entrypoint:state.entrypoint,ready:action.payload.isReady};}case platformfrom_dll_reference_platform_dll.constants.SET_ENTRYPOINT_MODULE:{return{language:state.language,entrypoint:action.payload.entrypoint,ready:state.ready};}default:{return state;}}});
...
var getEntrypoint=function getEntrypoint(state){return state.runtime.entrypoint;};var getIsReady=function getIsReady(state){return state.runtime.ready;};var getLanguage=function getLanguage(state){return state.runtime.language;};
;// CONCATENATED MODULE: ./node_modules/@lastui/rocker/runtime/component/Localisation.jsx
var Localisation=function Localisation(props){var language=(0,libfrom_dll_reference_dependencies_dll.useSelector)(getLanguage);return reactfrom_dll_reference_dependencies_dll.createElement(react_intlfrom_dll_reference_dependencies_dll.IntlProvider,{locale:language},props.children);};/* harmony default export */ const component_Localisation = (Localisation);
